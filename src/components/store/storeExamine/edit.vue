    ...
    export default {
        ...
        methods: {
            handleSubmit (name) {
                ...
                this.projectStr = str;
                //处理缴费金额
                if(this.marginPaymentStatus=='true'){
                    this.payStatus = true;
                }else{
                ...
                let url = common.path + 'store/modify'
                this.$http.put(
                    url,
            ...
            //获取数据
            getDataById(id){
                let url = common.path+'store/query/'+id;
                this.$http.get(url).then(res=>{
                    ...
                    this.marginPaymentStatus = storeExtend.marginPaymentStatus?'true':'false'; //保证金缴纳状态
